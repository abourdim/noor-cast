...
const ZIP_DIR = resolve(PKG, 'NoorCast-v0.7');
...
async function main() {
  ...
  const zipPath = join(PKG, 'NoorCast-v0.7.zip');
  try {
    execSync(`cd "${PKG}" && zip -r "${zipPath}" NoorCast-v0.7/`, { stdio: 'pipe' });
    console.log(`  ✅ ${zipPath}`);
  } catch {
    console.log('  ⚠️  zip command not available, trying ditto...');
    try {
      execSync(`cd "${PKG}" && ditto -c -k --keepParent NoorCast-v0.7 "${zipPath}"`, { stdio: 'pipe' });
      console.log(`  ✅ ${zipPath}`);
    } catch {
      console.log('  ❌ Could not create ZIP. Manually zip the NoorCast-v0.7 folder.');
    }
  }
